...
class personio
{
    ...
    public static function import()
    {
        ...
        $old_jobs = \D2U_Jobs\Job::getAllpersonioJobs();
        ...
        foreach ($xml_jobs->entry as $xml_job) {
            ...
            if ($xml_job->kopfgrafik_url != "") {
                ...
                if ($job_picture instanceof \rex_media && $job_picture->fileExists()) {
                    ...
                    if (copy($xml_job->kopfgrafik_url, $target_picture)) {
                        ...
                        $username = rex::getUser() ? rex::getUser()->getLogin() : "d2u_jobs_personio_plugin";
                        $sync_file_infos = \rex_mediapool_syncFile($job_picture_pathInfo['basename'], \rex_config::get('d2u_jobs', 'personio_media_category'), $xml_jobs->titel, null, null, $username);
            ...
            // Category
            $category = \D2U_Jobs\Category::getBypersonioID($xml_job->berufskategorie_id->__toString());
            if ($category === false) {
                self::log('Category with personio ID '. $xml_job->berufskategorie_id->__toString() .' does not exist. Falback to default category.');
                $category = new \D2U_Jobs\Category(\rex_config::get('d2u_jobs', 'personio_default_category'), \rex_config::get('d2u_jobs', 'personio_default_lang'));
            }

            // Import job
            $job = \D2U_Jobs\Job::getBypersonioID($xml_job->jobid->__toString());
            if ($job == false) {
                $job = \D2U_Jobs\Job::factory();
                $job->clang_id = \rex_config::get('d2u_jobs', 'personio_default_lang');
                $job->personio_job_id = $xml_job->jobid->__toString();
            }
            ...
            $job->personio_lead_in = $xml_job->einleitung->__toString();
            $job->personio_url_application_form = $xml_job->url_application_form->__toString();
